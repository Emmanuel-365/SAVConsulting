...
export function TrustBar() {
  const tools = [
    ...
    { name: "ACD Groupe", logo: "/logos/acd.png" },
    ...
    { name: "Fulll", logo: "/logos/fulll.webp" },
    { name: "Yooz", logo: "/logos/yooz.svg" },
    { name: "Open Paie", logo: "/logos/openpaie.svg" },
  ];

  return (
    <section className="py-8 md:py-10 border-y bg-slate-50/50 overflow-hidden">
      <div className="container mx-auto px-4">
        <p className="text-center text-[10px] md:text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-6 md:mb-8">
        ...
        <div className="relative flex overflow-x-hidden group">
          ...
          {/* Vitesse : 10s sur mobile (rapide), 30s sur desktop (calme) */}
          <div className="animate-marquee [animation-duration:15s] md:[animation-duration:35s] whitespace-nowrap flex items-center gap-8 md:gap-16 min-w-full">
            ...
            {[...tools, ...tools].map((tool, index) => (
              <div key={index} className="relative h-10 w-24 md:h-16 md:w-32 flex items-center justify-center grayscale opacity-60 hover:grayscale-0 hover:opacity-100 transition-all duration-300 shrink-0">
                <Image 
                  src={tool.logo} 
                  alt={`Logo ${tool.name}`} 
                  width={120}
